function [Xdn, sigma, npars, u, vals, v] = MP(X, nbins)
    ...
    % RS: Modified to provide output of SVD
    ...
    [M, N] = size(X); % M = timepoints; N = pixels; X = data
    ...
    [u, vals, v] = svd(X, 'econ');
    vals = diag(vals).^2 / N;   % to get percent fraction explained variance
    ...
    rangeMP = 4*sqrt(gamma(:)); %Marchenko pastur EVs
    ...
    vals(t:R) = 0;
    vals = sqrt(N*vals);
    Xdn = u*diag(vals)*v'; % Reproject onto non-noise principle components
